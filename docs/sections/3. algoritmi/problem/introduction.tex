...
We distinguish between small and big jobs based on their processing times. We defer the exact definition of small and big jobs to a later section (see Section~\ref{subsubsec:big-jobs}), as it relies on the approximation parameter $\epsilon$ in the context of profiling.
...
Let's begin by discussing the \emph{size of the problem}, which is arguably the easier influential factor to visualize. We approach this by distinguishing first the growth in size, and then the ratio between jobs and machines.
...
\begin{itemize}
  ...
  \item \textbf{EASY -- Jobs abundance} ($n/m \rightarrow \infty$): Many jobs compete for a limited number of machines, leading to machine saturation. In this regime, most machines are heavily loaded regardless of the specific assignment, and many schedules are near-optimal.
\end{itemize}
...
\\
If we consider the case of identical machines, which is the main focus of this project, the above heuristic needs to be adjusted. In fact, when $n/m = 1$, each machine receives exactly one job, yielding a trivial instance that is easy to solve%
