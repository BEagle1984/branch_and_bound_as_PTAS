\section{Solution approximation}
...
For the sake of simplicity and consistency with the branch-and-bound PTAS framework introduced by Encz, Mastrolilli, and Vercesi~\cite{bb-ptas}, we introduce the variable $\varepsilon > 0$ such that $\rho = 1 + \varepsilon$.
...
Therefore, an algorithm is a $(1 + \varepsilon)$-approximation if it guarantees that $C_{\mathcal{A}} \leq (1 + \varepsilon) \cdot C_{OPT}$, namely that the solution produced is at most $\varepsilon$ times worse than the optimal solution.
...
Approximation schemes categorize algorithms based on their computational complexity, thereby providing a means of comparison between different approaches.
...
\begin{itemize}
  \item \textbf{PTAS (Polynomial Time Approximation Scheme)}: An algorithm that, for every positive $\varepsilon$ value, produces a solution that is within a factor of $(1 + \varepsilon)$ of the optimum in polynomial time relative to the input size.
  However, the execution time may depend exponentially (or worse) on $1/\varepsilon$.
  \item \textbf{FPTAS (Fully Polynomial Time Approximation Scheme)}: An algorithm that, for every positive $\varepsilon$ value, produces a solution within a factor of $(1 + \varepsilon)$ of the optimum in polynomial time with respect to both the input size and $1/\varepsilon$.
\end{itemize}
...
The difference between the two can be better perceived by looking at the case when a very accurate approximation is desired (i.e. $\varepsilon$ very small, $\varepsilon\rightarrow 0$): in this case,
\begin{itemize}
